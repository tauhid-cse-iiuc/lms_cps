...
export function LessonManager({
  ...
  return (
    <div className="mt-4">
      ...
      {lessons.length === 0 ? (
        <p className="text-small text-ink-600">No lessons yet.</p>
      ) : (
        <ul className="space-y-2">
          {lessons.map((lesson) => (
            <li key={lesson.documentId} className="rounded border border-ink-200 p-3">
              {editing === lesson.documentId ? (
                <form onSubmit={(e) => handleUpdate(lesson.documentId, e)} className="space-y-3">
                  ...
                  <div className="flex gap-3">
                    <button
                      type="submit"
                      disabled={pending}
                      className="rounded bg-ink-900 px-3 py-1.5 text-small text-white disabled:opacity-50"
                    >
                      Save
                    </button>
                    <button
                ...
                <div className="flex items-baseline justify-between gap-4">
                  <span className="text-small">
                    <span className="text-ink-500">{lesson.order}.</span> {lesson.title}
                  </span>
      ...
      <form onSubmit={handleCreate} className="mt-6 space-y-3 rounded border border-ink-200 p-4">
        <h3 className="text-small font-medium">Add a lesson</h3>
        <label className="block text-small font-medium text-ink-700">
          Title
